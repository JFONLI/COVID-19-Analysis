...
count_death_bar <- filter(count_death_bar, sex == "male")
count_death_bar$sex %>% unique()
count_death_bar
ggplot(count_death_bar) +
  geom_col(aes(y = reorder(disease_type, death_by_disease), x = death_by_disease, fill = age_group))

count_death_bar$percents <- count_death_bar$death_by_disease / count_death_bar$cause_all

count_death_bar <- count_death_bar %>% group_by(disease_type) %>% summarise(total_counts = sum(death_by_disease),
                                                         all_causes = cause_all)
...
count_death_bar$percents_100 <- count_death_bar$total_counts / count_death_bar$sum_of_diseases * 100
