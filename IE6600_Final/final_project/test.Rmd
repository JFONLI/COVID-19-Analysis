...
shades <- rev(shades)
loli$age_group <- as.character(loli$age_group)
loli$diseases <- as.character(loli$diseases)
sum(loli$deaths < 0)
loli$age_group %>% unique()

# loli<- loli %>% filter(age_group == "0-4 years")
loli$deaths

library(ggridges)
loli$age_group %>% unique
loli$age_group[loli$age_group == "0-4 years"] <- "0-4"
loli$age_group %>% unique
loli
...
ggplot(loli) +
  geom_col(aes(y = deaths, x = diseases, fill = age_group), alpha = 0.5) 
