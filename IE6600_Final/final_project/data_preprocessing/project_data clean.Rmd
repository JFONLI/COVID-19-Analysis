...
# take subtract of NaturalCause and sum of 12 to 23
rss_8 = count_death$NaturalCause - sum_columns$total
...
```
The above code shows that, column 12-22 and 24 are counted in Natural Cause and All Cause.

We also find more definition of Natural Cause of death:
  - https://en.wikipedia.org/wiki/Manner_of_death

In conclusion, we will consider Covid19 Undelying as a part of Natural Cause of death. 
...
```
compare value of column 'cause_other' and column 'COVID-19 (U071, Underlying Cause of Death)'
```{r}
count_death_c5 %>% 
  filter(., cause_other < `COVID-19 (U071, Underlying Cause of Death)`) %>% 
  select(., cause_other, `COVID-19 (U071, Underlying Cause of Death)`)
```
We can find that there are many columns where the value of column 
'COVID-19 (U071, Underlying Cause of Death)' is larger than column 'cause_other'. Thus, we can say the death of COVID-19 is counted as natural causes.

Mutate new column 'percent_covi' by row-wise division: 'COVID-19 (U071, Underlying Cause of Death)' / 'AllCause'
```{r}
# mutate new column 'percent_covi'
count_death_c5 = count_death_c5 %>% 
  rowwise() %>% 
  mutate(., percent_covi = 
           `COVID-19 (U071, Underlying Cause of Death)` / AllCause) %>% 
  ungroup()
...
# let's have a look at which group has the largest value of percentage of death due to COVID-19
count_death_c5 %>% 
  arrange(., desc(percent_covi)) %>% 
  select(., Sex, `Race/Ethnicity`, AgeGroup, percent_covi)
```
...
pivot disease-related columns 
  - Note: Be careful with the index of columns to be pivoted. To avoid wrong index, indexing using column names is recommended. 
```{r}
# pivot columns
count_death_c7_2 = pivot_longer(count_death_c6,
                              col=`Septicemia (A40-A41)`: `COVID-19 (U071, Underlying Cause of Death)`,
...
# cleaned table of count_death.csv for visualization(only pivoting column 'cause_all', 'cause_natural' and 'cause_oth')
count_death_cleand_visual_1 = count_death_c7_1

# cleaned table of count_death.csv for visualization(only pivoting column 'Septicemia (A40-A41)' to 'COVID-19 (U071, Underlying Cause of Death)')
count_death_cleand_visual_2 = count_death_c7_2
...
write_rds(count_death_cleand_visual_1, "./dataset/count_death_cleaned_visual_1.rds")
write_rds(count_death_cleand_visual_2, "./dataset/count_death_cleaned_visual_2.rds")
...
```{r}
read_rds('./dataset/count_death_cleaned_visual_1.rds') %>% str()
read_rds('./dataset/count_death_cleaned_visual_2.rds') %>% str()
