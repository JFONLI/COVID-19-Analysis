...
## TS 
```{r}
ts <- count_death_cleaned_visual_2
ts$disease_type %>% unique()

#  [1] "Alzheimer disease (G30)"                                                                          
#  [2] "Cerebrovascular diseases (I60-I69)"                                                               
#  [3] "Chronic lower respiratory diseases (J40-J47)"                                                     
#  [4] "COVID-19 (U071, Underlying Cause of Death)"                                                       
...
# [10] "Other diseases of respiratory system (J00-J06,J30-J39,J67,J70-J98)"                               
# [11] "Septicemia (A40-A41)"                                                                             
# [12] "Symptoms, signs and abnormal clinical and laboratory findings, not elsewhere classified (R00-R99)"
...
ggplot(ts) +
  geom_line(aes(x = date_start, y = deaths, color = disease_type))
```
...
corrplot.mixed(cor.mat,  type='upper',  method = 'color',
         col.lim = c(0.1,1),is.corr = FALSE, tl.col = 'black', diag = TRUE, 
         addCoef.col = 'black', col = COL2('RdBu', 10),  
         tl.cex = 0.8, cl.cex = 0.70, number.cex = 0.7, tl.srt= 390)#, 
...
shades <- colorRampPalette(c("#605CA8", "#00C0EF", "#F39C11"))(10)
shades <- rev(shades)
...
condition_covi_cleaned$condition_group %>% unique()
```
# KPI Card
```{r}
...
p <- plot_usmap(data = map, values = "total_cases")
p
...
time_series <- time_series %>% filter(state == "California")
time_series
...
kpi_deaths <- round(kpi_deaths/332000000 * 100, 1)
...
ylim.cases <- c(0, max(temp$tot_cases))
ylim.deaths <- c(0, max(temp$new_case))

b <- diff(ylim.cases)/diff(ylim.deaths)
a <- ylim.cases[1] - b*ylim.deaths[1]

ggplot(temp, aes(x = date, y = tot_cases)) +
  geom_area(fill = "blue") +
  geom_area(aes(y = a + new_case*b), color = "red", fill = "red") +
